...
appinputs *gs::gai() const
{
	return ai;
}

siminputs *gs::gsi() const
{
	return si;
}

stat **gs::gstats() const
{
	return stats;
}
...
//        FIND CELL
//---------------------------------
int gs::findCell(const int *rImpl, const int &numImpl, const int &rVM, const double &rvProc,const double &rMem, const double &rNet, const double &rSto, const int *rAcc) const
{
	...
	for(i=0;i<si->numOfCells;i++)
	{
		for(k=0;k<numImpl;k++)
		{
			...
			if(rind==-1)
				continue;
			if(stats[i][rind].availProc>=((double)rVM)*rvProc && stats[i][rind].availMem>=((double)rVM)*rMem && stats[i][rind].availNetw>=rNet && stats[i][rind].availSto>=((double)rVM)*rSto && stats[i][rind].availAcc>=((double)rVM)*rAcc[k])
			{
				lweight=((stats[i][rind].availProc-((double)rVM)*rvProc)/(stats[i][rind].availProc+1)+(stats[i][rind].availMem-((double)rVM)*rMem)/(stats[i][rind].availMem+1)+(stats[i][rind].availNetw-rNet)/(stats[i][rind].availNetw+1)+(stats[i][rind].availSto-((double)rVM)*rSto)/(stats[i][rind].availSto+1)+(stats[i][rind].availAcc-((double)rVM)*rAcc[k])/(stats[i][rind].availAcc+1));
				if (lweight>weight)
				{
					weight=lweight;
					choice=i+1;
