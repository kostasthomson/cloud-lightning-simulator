...
improvedSosmBroker::improvedSosmBroker()
    : baseBroker(),
      numberOfvRMs(0),
      numberOfpSwitches(0),
      numberOfpRouters(0),
...
void improvedSosmBroker::init(const cell *clCell, const siminputs *si)
{
  ...
  for (int i = 0; i < numberOfTypes; i++)
  {
    ...
    tempP[i] = clCell->getPowerConsumption()[i].modelCPU(oz) /
               clCell->getResources()[i][0].getTotalProcessors();
    ...
    oz = 0.0;
    tempPi[i] = clCell->getPowerConsumption()[i].modelCPU(oz) /
                clCell->getResources()[i][0].getTotalProcessors();
  ...
  */
  ...
  numberOfvRMs = 0;
  numberOfpSwitches = 0;
  vRMs = new list<improvedvRM> *[numberOfTypes];
  pSwitches = new list<improvedpSwitch> *[numberOfTypes];
  pRouters = new list<improvedpRouter> *[numberOfTypes];

  for (int i = 0; i < numberOfTypes; i++)
  {
    vRMs[i] = new list<improvedvRM>[1];
    pSwitches[i] = new list<improvedpSwitch>[1];
    pRouters[i] = new list<improvedpRouter>[1];
  }
  ...
  for (int i = 0; i < numberOfTypes; i++)
  {
    ...
    for (int j = 0; j < temp; j++)
    {
      ...
      vRMs[i]->push_back(improvedvRM(j * si->cinp->binp[0].initResPervRM, (j + 1) * si->cinp->binp[0].initResPervRM, i,
                                     clCell->getResources(), pollIntervalvRM, tempC[i], tempCacc[i], tempP[i], tempPacc[i], tempPi[i],
                                     tempPiacc[i], si->cinp->binp[0].initResPervRM, si->cinp->binp[0].numberOfFunctions, si->cinp->binp[0].Ws,
                                     si->cinp->binp[0].vRMdeploystrategy));
    }
    numberOfvRMs += temp;
    if (temp * si->cinp->binp[0].initResPervRM != numberOfResourcesPerType[i])
    {
      ...
      //                         si->cinp->binp[0].initResPervRM, si->cinp->binp[0].numberOfFunctions, si->cinp->binp[0].Ws,
      //                         si->cinp->binp[0].vRMdeploystrategy));
      vRMs[i]->push_back(improvedvRM(temp * si->cinp->binp[0].initResPervRM, numberOfResourcesPerType[i], i,
                                     clCell->getResources(), pollIntervalvRM, tempC[i], tempCacc[i], tempP[i], tempPacc[i], tempPi[i],
                                     tempPiacc[i], si->cinp->binp[0].initResPervRM, si->cinp->binp[0].numberOfFunctions, si->cinp->binp[0].Ws,
                                     si->cinp->binp[0].vRMdeploystrategy));
      numberOfvRMs++;
  ...
  for (int i = 0; i < numberOfTypes; i++)
  {
    ...
    for (int j = 0; j < temp; j++)
    {
      ...
      pSwitches[i]->push_back(improvedpSwitch(
          j * si->cinp->binp[0].initvRMPerpSwitch, (j + 1) * si->cinp->binp[0].initvRMPerpSwitch, i, vRMs,
          pollIntervalpSwitch, tempC[i], tempCacc[i], tempP[i], tempPacc[i], tempPi[i], tempPiacc[i], si->cinp->binp[0].numberOfFunctions, si->cinp->binp[0].Ws));
    }
    ...
    if (temp * si->cinp->binp[0].initvRMPerpSwitch != ((int)vRMs[i]->size()))
    {
      ...
      pSwitches[i]->push_back(improvedpSwitch(temp * si->cinp->binp[0].initvRMPerpSwitch, ((int)vRMs[i]->size()), i, vRMs,
                                              pollIntervalpSwitch, tempC[i], tempCacc[i], tempP[i], tempPacc[i], tempPi[i],
                                              tempPiacc[i], si->cinp->binp[0].numberOfFunctions, si->cinp->binp[0].Ws));
      numberOfpSwitches++;
    }
  }
  ...
  for (int i = 0; i < numberOfTypes; i++)
  {
    ...
    pRouters[i]->push_back(improvedpRouter(0, ((int)pSwitches[i]->size()), i, pSwitches, pollIntervalpRouter, tempC[i], tempCacc[i],
                                           tempP[i], tempPacc[i], tempPi[i], tempPiacc[i], si->cinp->binp[0].numberOfFunctions, si->cinp->binp[0].Ws));
  }
  ...
  for (int i = 0; i < numberOfTypes; i++)
  {
    for (list<improvedpRouter>::iterator it = pRouters[i]->begin(); it != pRouters[i]->end(); it++)
    {
      for (int j = 0; j < 8; j++)
      {
...
/*
improvedSosmBroker::improvedSosmBroker(const int &L_numberOfTypes, const int *L_types, const int *L_numberOfResourcesPerType, resource
**resources,
...
*/
improvedSosmBroker::improvedSosmBroker(const improvedSosmBroker &t)
{
  int i, j;
  if (t.galloc())
  {
    ...
    for (i = 0; i < numberOfTypes; i++)
    {
      for (list<improvedvRM>::iterator it = t.getvRMs()[i]->begin(); it != t.getvRMs()[i]->end(); it++)
      {
        vRMs[i]->push_back(*it);
      }
    }
    ...
    for (list<improvedpSwitch>::iterator it = t.gpSwitches()[i]->begin(); it != t.gpSwitches()[i]->end(); it++)
    {
    ...
    for (list<improvedpRouter>::iterator it = t.gpRouters()[i]->begin(); it != t.gpRouters()[i]->end(); it++)
    {
...
improvedSosmBroker &improvedSosmBroker::operator=(const improvedSosmBroker &t)
{
  int i, j;
  if (this != &t)
  {
...
void improvedSosmBroker::print() const
{
  if (alloc)
  {
    for (int i = 0; i < numberOfTypes; i++)
...
double improvedSosmBroker::deassessmentFunctions(const double &dNu, const double &dAcc, const int &choice, const int &type)
{
  switch (choice)
  {
  case 0:
  {
    double Ln = (Pis[type] * (sPMSA[type][1]) + (Ps[type] - Pis[type]) * (sPMSA[type][1] - sPMSA[type][0]) + Piaccs[type] * (sPMSA[type][6]) +
                 Paccs[type] * (sPMSA[type][7] - sPMSA[type][6]));
    double Kn = Cs[type] * sPMSA[type][1] + Caccs[type] * sPMSA[type][7];
    return (Kn * (dNu * (Ps[type] - Pis[type]) + dAcc * Paccs[type])) / (Ln * Ln);
  }
  break;
  default:
    return 0.0;
...
void improvedSosmBroker::updateStateInfo(const cell *clCell, const double &tstep)
{
  int i, j, k;
  list<improvedpRouter>::iterator ittt;
  list<improvedpSwitch>::iterator itt;
  list<improvedvRM>::iterator it;
  if (alloc)
  {
...
#pragma omp parallel for default(shared) private(i, ittt) num_threads(omp_thr) schedule(static, 1)
    ...
    if (((int)tstep % (int)pollIntervalCellM) == 0)
    {
      for (i = 0; i < numberOfTypes; i++)
      {
        for (list<improvedpRouter>::iterator itttt = pRouters[i]->begin(); itttt != pRouters[i]->end(); itttt++)
        {
          for (j = 0; j < 8; j++)
...
void improvedSosmBroker::deploy(resource **resources, netw *network, stat *stats, task &_task)
{
  ...
  {
    list<improvedpRouter>::iterator itt = pRouters[rem[i]]->begin();
    ...
    if (maxSI < ((Cs[rem[i]] * (_task.greqPMNS()[0]) + (_task.grhoAcc()[i]) * Caccs[rem[i]]) / (Cs[rem[i]] * (_task.greqPMNS()[0]) + Caccs[rem[i]])) * SIs[rem[i]] &&
        _task.getNumberOfVMs() * _task.greqPMNS()[0] <= sPMSA[rem[i]][0] && _task.getNumberOfVMs() * _task.greqPMNS()[1] <= sPMSA[rem[i]][2] &&
        _task.getNumberOfVMs() * _task.greqPMNS()[3] <= sPMSA[rem[i]][4] && _task.getNumberOfVMs() * _task.gavAcc()[rem2[i]] <= sPMSA[rem[i]][6])
    {
      if (itt->probe(_task.getNumberOfVMs() * _task.greqPMNS()[0], _task.getNumberOfVMs() * _task.greqPMNS()[1],
                     _task.getNumberOfVMs() * _task.greqPMNS()[3], _task.getNumberOfVMs() * _task.gavAcc()[rem2[i]]) != -1)
      {
        maxSI = ((Cs[rem[i]] * (_task.greqPMNS()[0]) + (_task.grhoAcc()[i]) * Caccs[rem[i]]) / (Cs[rem[i]] * (_task.greqPMNS()[0]) + Caccs[rem[i]])) * SIs[rem[i]];
        type = i;
      }
    }
  }
  ...
  for (int i = 0; i < 4; i++)
  {
    SIs[type] += Ws[i] * deassessmentFunctions(-_task.getNumberOfVMs() * _task.greqPMNS()[0],
                                               -_task.getNumberOfVMs() * _task.gavAcc()[0], i, type);
  }
...
void improvedSosmBroker::timestep(const cell *clCell)
{
  ...
  list<improvedvRM>::iterator itt, **itf;
  ...
  if (alloc)
  {
    ...
    // OpenMP threads
    itf = new list<improvedvRM>::iterator *[numberOfTypes];
    for (i = 0; i < numberOfTypes; i++)
    {
      itf[i] = new list<improvedvRM>::iterator[omp_thr];
    }
    for (i = 0; i < numberOfTypes; i++)
    {
...
int improvedSosmBroker::getNumberOfvRMs() const { return numberOfvRMs; }
int improvedSosmBroker::getNumberOfpSwitches() const { return numberOfpSwitches; }
int improvedSosmBroker::getNumberOfpRouters() const { return numberOfpRouters; }
double improvedSosmBroker::gpollIntervalCellM() const { return pollIntervalCellM; }
double improvedSosmBroker::gpollIntervalpRouter() const { return pollIntervalpRouter; }
double improvedSosmBroker::gpollIntervalpSwitch() const { return pollIntervalpSwitch; }
double improvedSosmBroker::gpollIntervalvRM() const { return pollIntervalvRM; }
double **improvedSosmBroker::gsPMSA() const { return sPMSA; }
double *improvedSosmBroker::gSIs() const { return SIs; }
double *improvedSosmBroker::gCs() const { return Cs; }
double *improvedSosmBroker::gPs() const { return Ps; }
double *improvedSosmBroker::gPis() const { return Pis; }
double *improvedSosmBroker::gWs() const { return Ws; }
double *improvedSosmBroker::gCaccs() const { return Caccs; }
double *improvedSosmBroker::gPaccs() const { return Paccs; }
double *improvedSosmBroker::gPiaccs() const { return Piaccs; }
int improvedSosmBroker::getNumberOfFunctions() const { return numberOfFunctions; }
list<improvedvRM> **improvedSosmBroker::getvRMs() const { return vRMs; }
list<improvedpSwitch> **improvedSosmBroker::gpSwitches() const { return pSwitches; }
list<improvedpRouter> **improvedSosmBroker::gpRouters() const { return pRouters; }
