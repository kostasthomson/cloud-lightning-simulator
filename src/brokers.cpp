...
pRouter::pRouter()
  : alloc(0),
    ...
    totalProcessors(NULL),
    availableMemory(NULL),
    totalMemory(NULL),
    availableAccelerators(NULL),
    totalAccelerators(NULL),
...
pRouter::pRouter(const pRouter& t)
{
  if (t.galloc()) {
    alloc = 1;
    numberOfpSwitches = t.gnumberOfpSwitches();
...
{
  if (alloc) {
    int L_numberOfVMs = t->getNumberOfVMs();
...
#pragma omp for
    ...
    if (choice == -1) {
      stats[t->getAvailableImplementations()[0]].rejectedTasks++;
      return;
    }
...
int pRouter::gnumberOfpSwitches() const { return numberOfpSwitches; }
...
#pragma omp parallel default(shared) private(i, tid) num_threads(omp_thr)
      ...
      if (ores.size() > 0) {
        ...
        if (sPMSA[7] > 0)
          ...
          for (j = 0; j < 4; j++) {
            ssum += Ws[j] * deassessmentFunctions(((double)ores.size()) * ((*pores.begin())->getTotalProcessors()),
                                                  totalProcessors[choice],
                                                  ((double)ores.size()) * ((*ores.begin())->getTotalMemory()),
                                                  totalMemory[choice], j);
          }
  ...
  if (alloc) {
    ...
    while (it != res->end()) {
      ...
      if ((*it)->getMovable() == 1) {
        ...
        availableProcessors2 = NULL;
        totalProcessors2 = NULL;
        availableMemory2 = NULL;
        totalMemory2 = NULL;
        availableAccelerators2 = NULL;
        totalAccelerators2 = NULL;
        availableStorage2 = NULL;
        totalStorage2 = NULL;
        computeFs();
        computeSI();
        it = res->erase(it);
...
void vRM::deploy(resource** resources, netw* network, stat* stats, task* t)
{
  if (alloc) {
    int L_numberOfVMs = t->getNumberOfVMs();
    int type = t->getAvailableImplementations()[0];
    double* L_reqPMNS = t->greqPMNS();
    double ssum = 0.0;
    int L_avAcc = t->gavAcc()[0];
    int L_ID = -1;
...
void broker::initBroker(const int& L_numberOfTypes, const int* L_types, const int* L_numberOfResourcesPerType,
                        resource** resources, power* powerComp, netw* network, const brinputs& binp)
{
  alloc = 1;
  ...
  for (int i = 0; i < numberOfTypes; i++) {
    ...
    for (int j = 0; j < temp; j++) {
      vRMs[i]->push_back(vRM(j * binp.initResPervRM, (j + 1) * binp.initResPervRM, i, resources, pollIntervalvRM,
                             tempC[i], tempP[i], tempPi[i], binp.initResPervRM, binp.numberOfFunctions, binp.Ws,
                             binp.vRMdeploystrategy));
    }
    numberOfvRMs += temp;
    if (temp * binp.initResPervRM != numberOfResourcesPerType[i]) {
      vRMs[i]->push_back(vRM(temp * binp.initResPervRM, numberOfResourcesPerType[i], i, resources, pollIntervalvRM,
                             tempC[i], tempP[i], tempPi[i], binp.initResPervRM, binp.numberOfFunctions, binp.Ws,
                             binp.vRMdeploystrategy));
      numberOfvRMs++;
    }
  ...
  for (int i = 0; i < numberOfTypes; i++) {
    ...
    for (int j = 0; j < temp; j++) {
      pSwitches[i]->push_back(pSwitch(j * binp.initvRMPerpSwitch, (j + 1) * binp.initvRMPerpSwitch, i, vRMs,
                                      pollIntervalpSwitch, tempC[i], tempP[i], tempPi[i], binp.numberOfFunctions,
                                      binp.Ws));
    }
    numberOfpSwitches += temp;
    if (temp * binp.initvRMPerpSwitch != ((int)vRMs[i]->size())) {
      pSwitches[i]->push_back(pSwitch(temp * binp.initvRMPerpSwitch, ((int)vRMs[i]->size()), i, vRMs,
                                      pollIntervalpSwitch, tempC[i], tempP[i], tempPi[i], binp.numberOfFunctions,
                                      binp.Ws));
      numberOfpSwitches++;
    }
  ...
  for (int i = 0; i < numberOfTypes; i++) {
    pRouters[i]->push_back(pRouter(0, ((int)pSwitches[i]->size()), i, pSwitches, pollIntervalpRouter, tempC[i],
                                   tempP[i], tempPi[i], binp.numberOfFunctions, binp.Ws));
  }
...
/*
broker::broker(const int &L_numberOfTypes, const int *L_types, const int *L_numberOfResourcesPerType, resource
**resources,
...
broker::broker(const broker& t)
{
  ...
  if (t.galloc()) {
    ...
    pollIntervalpSwitch = t.gpollIntervalpSwitch();
    pollIntervalvRM = t.gpollIntervalvRM();
    numberOfpRouters = t.gnumberOfpRouters();
    numberOfpSwitches = t.gnumberOfpSwitches();
    numberOfvRMs = t.getNumberOfvRMs();
    types = new int[numberOfTypes];
    numberOfResourcesPerType = new int[numberOfTypes];
    ...
    totalNetwork = t.getTotalNetworkork();
    SIs = new double[numberOfTypes];
...
double broker::deassessmentFunctions(const double& dNu, const double& dNmem, const int& choice, const int& type)
{
  if (sPMSA[type][7] > 0) {
    switch (choice) {
      case 0:
...
{
  int type = -1, i, j;
  int L_numberOfVMs, *L_availableImplementations, *rem, *rem2, count;
  double maxSI;
  L_availableImplementations = t->getAvailableImplementations();

  rem = new int[numberOfTypes];
  rem2 = new int[t->getNumberOfAvailableImplementations()];
  count = 0;

  // Determine how many (count) of the pRouter hardware types match the tasks' requested type, and store the types to
  // the rem and rem arrays
  for (j = 0; j < t->getNumberOfAvailableImplementations(); j++) {
    for (i = 0; i < numberOfTypes; i++) {
      if (types[i] == L_availableImplementations[j]) {
        rem[count] = i;
        rem2[count] = j;
        count++;
        break;
  ...
  if (availableNetwork < t->greqPMNS()[2]) {
  ...
  maxSI = 0.0;
  type = -1;
  L_numberOfVMs = t->getNumberOfVMs();
  ...
  for (i = 0; i < count; i++) {
    ...
    if (maxSI < SIs[rem[i]] && L_numberOfVMs * t->greqPMNS()[0] <= sPMSA[rem[i]][0] &&
        L_numberOfVMs * t->greqPMNS()[1] <= sPMSA[rem[i]][2] && L_numberOfVMs * t->greqPMNS()[3] <= sPMSA[rem[i]][4] &&
        L_numberOfVMs * t->gavAcc()[rem2[i]] <= sPMSA[rem[i]][6]) {
      if (itt->probe(L_numberOfVMs * t->greqPMNS()[0], L_numberOfVMs * t->greqPMNS()[1],
                     L_numberOfVMs * t->greqPMNS()[3], L_numberOfVMs * t->gavAcc()[rem2[i]]) != -1) {
        maxSI = SIs[rem[i]];
        type = i;
      }
    }
  ...
  type = rem[type];
  sPMSA[type][0] -= L_numberOfVMs * t->greqPMNS()[0];
  sPMSA[type][2] -= L_numberOfVMs * t->greqPMNS()[1];
  sPMSA[type][4] -= L_numberOfVMs * t->greqPMNS()[3];
  sPMSA[type][6] -= L_numberOfVMs * t->gavAcc()[0];

  // Update SI for the chosen pRouter
  for (i = 0; i < 4; i++) {
    SIs[type] +=
      Ws[i] * deassessmentFunctions(-L_numberOfVMs * t->greqPMNS()[0], -L_numberOfVMs * t->greqPMNS()[1], i, type);
  }
        ...
        for (k = (tid * len) / omp_thr; k < ((tid + 1) * len) / omp_thr; k++) {
          ...
          for (ittt = (itt->gqueue())->begin(); ittt != (itt->gqueue())->end(); ittt++) {
            ...
            for (j = 0; j < L_numberOfVMs; j++) {
              ...
              resources[i][rID].incrementRunningQuantities(gcU[0], gcU[1], gcU[3]);
            }
          }
          itt++;
        }
#pragma omp barrier
    ...
    network[0].incrementRunningQuantities(L_net[0]);
    delete[] L_net;
...
#pragma omp parallel for default(shared) private(j) num_threads(omp_thr) schedule(static, chunk)
      for (j = 0; j < numberOfResourcesPerType[i]; j++) {
        if (resources[i][j].getNumberOfTasks() > 0) {
          resources[i][j].compcurrentCompCapPerProc();
          resources[i][j].compcurrentCompCapPerAcc();
        }
      }
    }
    // Step 4
...
#pragma omp parallel for default(shared) private(j, procUtil, rhoAcc, active, totalAcc) num_threads(omp_thr) \
      ...
      for (j = 0; j < numberOfResourcesPerType[i]; j++) {
        procUtil = resources[i][j].getActualUtilizedProcessors() / resources[i][j].getTotalProcessors();
        rhoAcc = resources[i][j].getActualRhoAccelerators();
        active = resources[i][j].getActive();
        totalAcc = resources[i][j].getTotalAccelerators();
        L_totalPowerConsumption += powerComp[i].consumption(procUtil, rhoAcc, active, totalAcc);
      }
      stats[i].totalPowerConsumption += L_totalPowerConsumption;
    }
    // Step 5
    ...
    for (i = 0; i < numberOfTypes; i++) {
      ocP = resources[i][0].getOvercommitmentProcessors();
      len = (int)vRMs[i]->size();
// For all vRms..
                                             ...
                                             L_numberOfVMs) num_threads(omp_thr)
      {
        ...
        for (jj = (tid * len) / omp_thr; jj < ((tid + 1) * len) / omp_thr; jj++) {
          for (ittt = itt->gqueue()->begin(); ittt != itt->gqueue()->end(); ittt++) {
            ...
            insR = resources[i][rID].getCurrentCompCapPerProc();
            insRa = resources[i][rID].getCurrentCompCapPerAcc();
            L_numberOfVMs = ittt->getNumberOfVMs();
            ...
            for (j = 1; j < L_numberOfVMs; j++) {
              ...
              // value
              insR = MY_MIN(insR, resources[i][rID].getCurrentCompCapPerProc());
              ...
              insRa = MY_MIN(insRa, resources[i][rID].getCurrentCompCapPerAcc());
            }
            // Reduce the instructions left to compute of the given task
            ittt->reduceIns(L_numberOfVMs * insR * MY_MIN(ittt->gcUtilPMNr()[0] * ocP, 1.0) +
...
#pragma omp barrier
      ...
      network[0].unload(avau[0][0], avau[0][1], numberOfTasks[0]);
    }
    for (i = 0; i < omp_thr; i++) {
      delete[] avau[i];
    }
...
int broker::galloc() const { return alloc; }
int broker::getNumberOfTypes() const { return numberOfTypes; }
int* broker::getTypes() const { return types; }
int* broker::getNumberOfResourcesPerType() const { return numberOfResourcesPerType; }
int broker::getNumberOfvRMs() const { return numberOfvRMs; }
int broker::gnumberOfpSwitches() const { return numberOfpSwitches; }
int broker::gnumberOfpRouters() const { return numberOfpRouters; }
double broker::gpollIntervalCellM() const { return pollIntervalCellM; }
double broker::gpollIntervalpRouter() const { return pollIntervalpRouter; }
double broker::gpollIntervalpSwitch() const { return pollIntervalpSwitch; }
double broker::gpollIntervalvRM() const { return pollIntervalvRM; }
double broker::getTotalNetworkork() const { return totalNetwork; }
double broker::getAvailableNetwork() const { return availableNetwork; }
double** broker::gsPMSA() const { return sPMSA; }
double* broker::gSIs() const { return SIs; }
double* broker::gCs() const { return Cs; }
double* broker::gPs() const { return Ps; }
double* broker::gPis() const { return Pis; }
double* broker::gWs() const { return Ws; }
int broker::getNumberOfFunctions() const { return numberOfFunctions; }
list<vRM>** broker::getvRMs() const { return vRMs; }
list<pSwitch>** broker::gpSwitches() const { return pSwitches; }
list<pRouter>** broker::gpRouters() const { return pRouters; }
...
traditionalBroker::traditionalBroker(const int& L_numOfTypes, const int* L_types, const int* L_numOfResourcesPerType,
                                     const double& L_pollInterval)
{
  int i = 0;
  alloc = 1;
...
void traditionalBroker::updateStateInfo(resource** resources, netw* network, const double& tstep)
{
  int i, j;
  int omp_thr = atoi(getenv("OMP_NUM_THREADS"));
  if (alloc) {
...
#pragma omp parallel for default(shared) private(j) num_threads(omp_thr) schedule(static)
        for (j = 0; j < numOfResourcesPerType[i]; j++) {
          availProc[i][j] = resources[i][j].getAvailableProcessors();
          totProc[i][j] = resources[i][j].getTotalProcessors();
          availMem[i][j] = resources[i][j].getAvailableMemory();
          totMem[i][j] = resources[i][j].getTotalMemory();
          availAcc[i][j] = resources[i][j].getAvailableAccelerators();
          totAcc[i][j] = resources[i][j].getTotalAccelerators();
          availSto[i][j] = resources[i][j].getAvailableStorage();
          totSto[i][j] = resources[i][j].getTotalStorage();
        }
      }
      availNetw = network[0].getAvailableNetwork();
      totNetw = network[0].getTotalNetwork();
    }
  }
}

void traditionalBroker::deploy(resource** resources, netw* network, stat* stats, task* t)
{
  int type = -1, i, j;
  int* IDs;
  int tID;
  ...
  reqPMNS = t->greqPMNS();
  avAcc = t->gavAcc()[0];
  L_availImpl = t->getAvailableImplementations()[0];
  L_numberOfVMs = t->getNumberOfVMs();
  int omp_thr = atoi(getenv("OMP_NUM_THREADS"));
  for (i = 0; i < numOfTypes; i++) {
  ...
  /*	for(j=0;j<L_numberOfVMs;j++)
    {
      ...
      {
        if(availProc[type][i]>=reqPMNS[0] && availMem[type][i]>=reqPMNS[1] && availSto[type][i]>=reqPMNS[3] &&
...
void traditionalBroker::enque(const task* t)
{
  if (alloc) {
    queue->push_back(*t);
  }
...
void traditionalBroker::timestep(resource** resources, netw* network, stat* stats, power* powerComp)
{
  int i, j, rID, type = -1;
  double insR, insRa;
  double procUtil;
...
#pragma omp parallel for default(shared) private(j) num_threads(omp_thr) schedule(static, chunk)
    ...
    for (it = queue->begin(); it != queue->end(); it++) {
      ...
      for (j = 0; j < it->getNumberOfVMs(); j++) {
        ...
        resources[type][rID].incrementRunningQuantities(it->gcUtilPMNr()[0], it->gcUtilPMNr()[1], it->gcUtilPMNr()[3]);
      }
      // network[0].incrementRunningQuantities(it->gcUtilPMNr()[2]);
    }
    network[0].incrementRunningQuantities(L_net);
...
#pragma omp parallel default(shared) private(i, it, j, rID, insR, insRa, type, L_numberOfVMs) num_threads(omp_thr)
    {
      ...
      for (i = 0; i < tid; i++)
        it++;
      i = tid;
      double ocP = resources[type][0].getOvercommitmentProcessors();
      while (i < len) {
        type = (it->getAvailableImplementations())[0];

        rID = (it->gresourceIDs())[0];
        insR = resources[type][rID].getCurrentCompCapPerProc();
        insRa = resources[type][rID].getCurrentCompCapPerAcc();
        L_numberOfVMs = it->getNumberOfVMs();
        for (j = 1; j < L_numberOfVMs; j++) {
          rID = (it->gresourceIDs())[j];
          insR = MY_MIN(insR, resources[type][rID].getCurrentCompCapPerProc());
          insRa = MY_MIN(insRa, resources[type][rID].getCurrentCompCapPerAcc());
        }
        it->reduceIns(L_numberOfVMs * insR * MY_MIN(it->gcUtilPMNr()[0] * ocP, 1.0) +
    ...
    while (it != queue->end()) {
      if ((it->grequestedInstructions()) <= 0.0) {
        type = (it->getAvailableImplementations())[0]; L_numberOfVMs = it->getNumberOfVMs();
        for (j = 0; j < L_numberOfVMs; j++) {
          rID = (it->gresourceIDs())[j];
...
int traditionalBroker::galloc() const { return alloc; }
double traditionalBroker::gpollInterval() const { return pollInterval; }
int traditionalBroker::gnumOfTypes() const { return numOfTypes; }
int* traditionalBroker::gtypes() const { return types; }
int* traditionalBroker::gnumOfResourcesPerType() const { return numOfResourcesPerType; }
double** traditionalBroker::gavailProc() const { return availProc; }
double** traditionalBroker::gtotProc() const { return totProc; }
double** traditionalBroker::gavailMem() const { return availMem; }
double** traditionalBroker::gtotMem() const { return totMem; }
double** traditionalBroker::gavailAcc() const { return availAcc; }
double** traditionalBroker::gtotAcc() const { return totAcc; }
double** traditionalBroker::gavailSto() const { return availSto; }
double** traditionalBroker::gtotSto() const { return totSto; }
double traditionalBroker::gavailNetw() const { return availNetw; }
double traditionalBroker::gtotNetw() const { return totNetw; }
