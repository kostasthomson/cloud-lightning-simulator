...
void taskCreationEngine(std::list<task>& jobs, const struct appinputs& app)
{
  ...
  int numOfTasks = round(app.minmaxJobsPerSec[0] + r * (app.minmaxJobsPerSec[1] - app.minmaxJobsPerSec[0]));
  ...
  for (int i = 0; i < numOfTasks; i++) {
    ...
    int curr_numOfVMs = round(app.minmaxVMPerApp[curr_task][0] +
    ...
    jobs.push_back(task(curr_task, app.numOfAvailImplPerApp[curr_task], &app.availImplPerApp[curr_task][0], curr_reqIns,
                        curr_numOfVMs, curr_reqP, curr_reqM, curr_reqN, curr_reqS, app.typeOfActP[curr_task],
...
void taskImplSelect(std::list<task>& jobs)
{
  ...
  for (i = 0; i < numtasks; i++) {
    ...
    (*it) = (task(L_type, L_numOfAvailImpl, L_availImpl, L_reqIns, L_numOfVMs, L_reqPMNS[0], L_reqPMNS[1], L_reqPMNS[2],
...
void taskCellSelect(std::list<task>& jobs, const gs* gates, int** commCells)
{
  ...
  if (jobs.size() != 0 && gates->galloc()) {
    ...
    for (int i = 0; i < (int)jobs.size(); i++) {
      ...
      (*commCells)[i] =
        gates->findCell((*it).gavailImpl(), (*it).gnumOfAvailImpl(), (*it).gnumOfVMs(), (*it).greqPMNS()[0],
