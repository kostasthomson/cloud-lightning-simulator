	...
	switch (typeCpu)
	{
		...
		case 1:
			if(u<cpubins[0])
				...
				pcons=cpuP[numOfPoints-2]+(cpuP[numOfPoints-1]-cpuP[numOfPoints-2])*(u-cpubins[numOfPoints-2])/(cpubins[numOfPoints-1]-cpubins[numOfPoints-2]);
			else
			{
		...
		case 2:
			if(u<cpubins[0])
				pcons=a[0]+b[0]*(u-cpubins[0])+c[0]*(u-cpubins[0])*(u-cpubins[0])+d[0]*(u-cpubins[0])*(u-cpubins[0])*(u-cpubins[0]);
			else if (u>cpubins[numOfPoints-1])
				pcons=a[numOfPoints-2]+b[numOfPoints-2]*(u-cpubins[numOfPoints-2])+c[numOfPoints-2]*(u-cpubins[numOfPoints-2])*(u-cpubins[numOfPoints-2])+d[numOfPoints-2]*(u-cpubins[numOfPoints-2])*(u-cpubins[numOfPoints-2])*(u-cpubins[numOfPoints-2]);
			else
			{
				for(i=1;i<numOfPoints;i++)
					if(u<=cpubins[i])
...
power::power(const powinputs &t)
{
	...
	else if (typeCpu>0)
	{
		...
		if (typeCpu==2)
		{
			...
			for(i=1;i<numOfPoints-1;i++)
				ta[i]=h[i-1];
			ta[numOfPoints-1]=-2.0*h[numOfPoints-2]*h[numOfPoints-2]-3.0*h[numOfPoints-3]*h[numOfPoints-2]-h[numOfPoints-3]*h[numOfPoints-3];
...
int power::galloc() const
{
	return alloc;
}

int power::gtypeCpu() const
{
	return typeCpu;
}

int power::gtypeAcc() const
{
	return typeAcc;
}

int power::gaccelerator() const
{
	return accelerator;
}

double power::gcpuPmin() const
{
	return cpuPmin;
}

double power::gcpuPmax() const
{
	return cpuPmax;
}

double power::gcpuC() const
{
	return cpuC;
}

int power::gnumOfPoints() const
{
	return numOfPoints;
}

double *power::gcpubins() const
{
	return cpubins;
}

double *power::gcpuP() const
{
	return cpuP;
}

double power::gaccPmin() const
{
	return accPmin;
}

double power::gaccPmax() const
{
	return accPmax;
}

double power::gaccC() const
{
	return accC;
}


double *power::ga() const
{
	return a;
}

double *power::gb() const
{
	return b;
}

double *power::gc() const
{
	return c;
}

double *power::gd() const
{
	return d;
}
