...
//        HELPERS
//---------------------------------
void task::reduceIns(const double & amount)
{
    reqIns-=amount;
}
...
int task::gtype() const
{
	return type;
}

int task::gnumOfAvailImpl() const
{
	return numOfAvailImpl;
}

int *task::gavailImpl() const
{
	return availImpl;
}

double task::greqIns() const
{
	return reqIns;
}

int task::gnumOfVMs() const
{
	return numOfVMs;
}

double *task::greqPMNS() const
{
	return reqPMNS;
}

int *task::gtypeactPMN() const
{
	return typeactPMN;
}

double **task::gminmaxactPMN() const
{
	return minmaxactPMN;
}

int *task::gavAcc() const
{
	return avAcc;
}

double *task::grhoAcc() const
{
	return rhoAcc;
}

int task::galloc() const
{
	return alloc;
}

int *task::gresourceIDs() const
{
	return resourceIDs;
}

double *task::gcUtilPMNr() const
{
	return cUtilPMNr;
}
...
void task::print() const
{
	...
	if(alloc)
	{
		...
		for(i=0;i<3;i++)
			cout<<typeactPMN[i]<<" ";
		cout<<endl;
		cout<<"Minimum - Maximum actual utilization Processors: "<<minmaxactPMN[0][0]<<" "<<minmaxactPMN[0][1]<<endl;
