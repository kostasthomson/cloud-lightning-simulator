...
class vRM
{
	...
	public:
		vRM();
		~vRM();
		vRM(const int &start,const int &end,const int &type,resource** resources, const double &L_pollIntervalvRM, const double &L_C, const double &L_P, const double &L_Pi, const int &L_optNumOfRes, const int &L_numOfFuncs,const double *L_Ws,const int &L_dep_strategy);
		vRM(const vRM &t);
		...
		int deploy_strategy(list<resource*>::iterator *it, int *IDs, const int &nVMs, const double &Proc, const double &Mem, const double &Sto, const int &Acc);
...
class pSwitch
{
	...
	public:
		pSwitch();
		~pSwitch();
		pSwitch(const int &start, const int &end, const int &type, list<vRM> **LvRMs,const double &pollIntervalpSwitch,const double &L_C, const double &L_P, const double &L_Pi, const int &L_numOfFuncs,const double *L_Ws);
		...
		void computeSI();
		void updateStateInfo(const double &tstep);
		double dassessfuncs(const double &dNu,const double &totNu, const double &dNmem, const double &totMem,const int &choice);
		int probe(const double &Proc, const double &Mem, const double &Sto, const int &Acc);
		void deploy(resource **resources, netw *network, stat* stats, task * t);
};
...
class pRouter
{
	...
	public:
		pRouter();
		~pRouter();
		pRouter(const int &start, const int &end, const int &type, list<pSwitch> **LpSwitches,const double &pollIntervalpRouter, const double &L_C, const double &L_P, const double &L_Pi, const int &L_numOfFuncs,const double *L_Ws);
...
class broker
{
	...
	public:
		broker();
		void initbroker(const int &L_numOfTypes, const int *L_types, const int *L_numOfResourcesPerType, resource **resources, power *powerComp, netw *network, const brinputs &binp);
		broker(const int &L_numOfTypes, const int *L_types, const int *L_numOfResourcesPerType, resource **resources, power *powerComp, netw *network, const brinputs &binp);
